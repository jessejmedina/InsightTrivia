#!/usr/bin/env node
/**
 * Dev helper: creates a waiting game room seeded with ONLY the four
 * ordering/matching questions from data/test-question-types.json, so you
 * can play the simultaneous-play phases deterministically instead of
 ...
 * You are the host -> tap "Start (Solo Dev Test)" to play all four solo.
 *
 * Re-run any time; it recreates the TESTTY room from scratch.
 */
const fs = require('fs');
...
const ROOM_CODE = 'TESTTY';

async function main() {
  const username = process.argv[2];
  if (!username) {
    console.error('Usage: node scripts/seed-test-room.js <your-username>');
    process.exit(1);
  }
  ...
  // 1. Resolve the four test questions by their exact question text.
  const testFile = JSON.parse(
    fs.readFileSync(path.join(__dirname, '..', 'data', 'test-question-types.json'), 'utf8')
  );
  ...
  // 3. Recreate the TESTTY room from scratch (cascade clears players/events).
  ...
  const { error: jErr } = await supabase
    .from('game_players')
    .insert({ room_id: room.id, user_id: hostId, team: null });
